...
# and without FEs
reg_equation(outcome_var = "le_15",
             regressor_vars = c("gdp_pc","tfr"))
...
## output using modelsummary ----
  ...
  title_crosssection <- "Life Expectancy Cross-Section Comparisons, 1950 and 2019 \\label{tab:lecrosssection}"
...
modelsummary(models,
  ...
  add_header_above(c(" "=1, "1950" =1, "2019"=1, "1950"=1, "2019"=1, "1950"=1,"2019"=1)) %>% # add a header that gives the years, a blank over the varnames then the other two spanning 3 columns
...
modelsummary(models,
  ...
  footnote(escape = FALSE, # this allows us to use \\\\citet{} and put citations in this end-of-table note
           general = my_notes,
           threeparttable = TRUE,
           general_title = "Notes: "
  )%>%
...
## set titles and notes ----
  ...
  out_path <- file.path(output_tables,"le_reg_panel.tex") # maybe hw03_reg_panel.tex
  title_paneltab <- "Panel Regression, Life Expectancy \\label{tab:lepanelregs}"
...
modelsummary(models,
  ...
  add_header_above(c(" "=1, "1950"=1, "2019" = 1, "Within"=1,"Twoway"=1))  # add a header that gives the years, a blank over the varnames then the other two spanning 3 columns


## output to latex ----
out_path <- file.path(output_tables,"le_reg_panel.tex")
...
modelsummary(models,
  ...
  footnote(escape = FALSE, # so that KableExtra knows to just pass through the \
