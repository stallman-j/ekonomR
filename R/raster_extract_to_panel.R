...
#' @param terra_raster a terra raster, currently required to be from 0-360 degrees longitude and -90 to 90 degrees latitude,
#' and must have a time dimension given
#' @param vector_sf an sf object with polygons, points or lines to be extracted to
#' @param save_raster_copy default TRUE, logical if you want to save a copy of the raster you bring in
#' with possibly updated coordinate reference system or rotated correctly
#' @param raster_out_path file path to save a copy of the raster to
#' @param raster_out_filename if saving a raster copy, name of the file
#' @param extracted_out_path filepath to where we output the data of extracting raster to vector
#' @param extracted_out_filename filename of output file from extracted raster to vector (no geometry kept)
#' @param vector_cast_out_path filepath to where the vector sf that gets casted is output
#' @param vector_cast_out_filename filename for the vector sf output
#' @param layer_substrings character vector, giving the substrings which identify what layers
#' assumes each element of this will give layers which generate the same longitudinal time sequence
#' @param func character vector, default "weighted_sum," function to be used for aggregating
#' @param weights character vector, default "area" to be used for weighting the function
...
raster_extract_to_panel <- function(terra_raster,
  ...
  # if CRS is different, transform to the polygon's to match the terra_raster's
  ...
  if (length(layer_substrings)==1){ # if only one substring don't bother with the loop
    ...
    if ("POINT" %in% vector_type | "MULTIPOINT" %in% vector_type ){
      ...
      out_df <- terra::extract(x = terra_raster,
        ...
        dplyr::pivot_longer(cols = !vector_cast_id,
                     names_to = "date",
