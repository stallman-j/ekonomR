...
The regression equation formula doesn't yet allow for complicated interactions yet of the sort `fixest` excels at, but it's good for outputting simple correlations and fixed effects regressions.
...
if (!dir.exists(here::here("output","01_tables"))) dir.create(here::here("output","01_tables"), recursive = TRUE)
 ...
 tinytable::save_tt(my_table,
                     output = here::here("output","01_tables","fixed_effects_table.tex"),
                     overwrite = TRUE)
#> Error: `j` must be integer-ish; all values must be less than or equal to 4
```
