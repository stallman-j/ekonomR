...
First, bring `ekonomR` into your working library. Let's also bring in `dplyr` explicitly so that the pipe operator comes through easily.
...
*Comprehension check*: What would happen if we told `pivot_longer()` to use *all* the columns, and didn't omit `year`? (If you're not sure, try it out with `cols = names(.)`.

## The placeholder period in dplyr

The `.` comes from how dplyr works with the pipe operator `%>%`. 

It's a place-holder that says "Hey, that thing that came before the pipe? Also insert it here where this period is." 
...
## Adjust data aesthetics with `dplyr::arrange()`
...
## Harmonize country names with `countrycode::countrycode()`

Country names are a little messy because different languages have different naming conventions for different places. ISO3C codes are a standardized way to refer to countries, although there are many more methodologies. The package `countrycode` has a function also called `countrycode()` that does this conversion for us. If you're doing a serious project, you should be careful to check that this is working as you intend. 
...
## Create/edit variables with `dplyr::mutate()`

We can create a new variable with `dplyr`'s `mutate()` function, which sounds like X-Men but really just gets used any time we want to create or change a variable's values. Within the `mutate()` function, we call the `countrycode()` function, saying to take the value from the country name that we got from `country_name`, and generate for us the ISO3C code which corresponds to it.
...
#> Error in eval(expr, envir, enclos): object 'gcb_temp2' not found
...
#> Error in eval(expr, envir, enclos): object 'gcb_temp3' not found
...
#> Warning in rm(gcb, gcb_temp, gcb_temp2, gcb_temp3, gcb_temp4): object 'gcb_temp2' not found
#> Warning in rm(gcb, gcb_temp, gcb_temp2, gcb_temp3, gcb_temp4): object 'gcb_temp3' not found
#> Warning in rm(gcb, gcb_temp, gcb_temp2, gcb_temp3, gcb_temp4): object 'gcb_temp4' not found
